<?php

namespace Tests\Feature\Employer;

use App\Models\Company;
use App\Models\JobListing;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class JobModuleTest extends TestCase
{
    use RefreshDatabase;

    public function test_employer_can_list_jobs()
    {
        $company = Company::factory()->create();
        JobListing::factory()->count(2)->create([
            'company_id' => $company->id,
        ]);

        $response = $this->getJson("api/employer/company/{$company->id}/jobs");

        $response->assertStatus(200)
                 ->assertJsonStructure([
                     'data' => [
                         ['id', 'title', 'description', 'company_id']
                     ]
                 ]);
    }

    public function test_employer_can_create_a_job()
    {
        $company = Company::factory()->create();

        $payload = [
            'title' => 'Backend Developer',
            'description' => 'Write APIs',
        ];

        $response = $this->postJson("api/employer/company/{$company->id}/jobs", $payload);

        $response->assertStatus(201)
                 ->assertJsonFragment(['title' => 'Backend Developer']);
    }

    public function test_employer_can_view_job_details()
    {
        $company = Company::factory()->create();
        $job = JobListing::factory()->create([
            'company_id' => $company->id
        ]);

        $response = $this->getJson("api/employer/company/{$company->id}/jobs/{$job->id}");

        $response->assertStatus(200)
                 ->assertJsonFragment(['id' => $job->id]);
    }

    public function test_employer_can_update_job()
    {
        $company = Company::factory()->create();
        $job = JobListing::factory()->create([
            'company_id' => $company->id
        ]);

        $updateData = [
            'title' => 'Updated Job Title'
        ];

        $response = $this->putJson(
            "api/employer/company/{$company->id}/jobs/{$job->id}", 
            $updateData
        );

        $response->assertStatus(200)
                 ->assertJsonFragment(['title' => 'Updated Job Title']);
    }

    public function test_employer_can_soft_delete_job()
    {
        $company = Company::factory()->create();
        $job = JobListing::factory()->create([
            'company_id' => $company->id
        ]);

        $response = $this->deleteJson("api/employer/company/{$company->id}/jobs/{$job->id}");

        $response->assertStatus(200);

        $this->assertSoftDeleted('job_listings', ['id' => $job->id]);
    }

    public function test_employer_can_restore_job()
    {
        $company = Company::factory()->create();
        $job = JobListing::factory()->create([
            'company_id' => $company->id,
            'deleted_at' => now(),
        ]);

        $response = $this->postJson("api/employer/company/{$company->id}/jobs/{$job->id}/restore");

        $response->assertStatus(200);

        $this->assertDatabaseHas('job_listings', [
            'id' => $job->id,
            'deleted_at' => null
        ]);
    }
}
